...
app.post("/build-usdz", async (req, res) => {
  try {
    ...
    // 2) Blender -> USD + texture.png in the SAME folder
    await run("blender", [
      "-b",
      "-P",
      "/app/make_usd.py",
    ...
    // Blender script writes: {id}.usd and texture.png next to it
    ...
    // 3) Package into .usdz = zip container with NO compression
    // We build a temp folder so the zip has clean paths.
    const pkgDir = path.join(WORK_DIR, `${id}_pkg`);
    fs.mkdirSync(pkgDir, { recursive: true });

    const usdPkgPath = path.join(pkgDir, `${id}.usd`);
    const texPkgPath = path.join(pkgDir, "texture.png");

    fs.copyFileSync(usdPath, usdPkgPath);
    fs.copyFileSync(texturePath, texPkgPath);

    // zip -0 (store) -r (recursive)
...
// Serve generated usdz files (lowercase path!)
app.use(
  "/usdz",
